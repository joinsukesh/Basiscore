...
namespace SitecoreCustomTools.sitecore.admin.minions
{
    ...
    public partial class publishitems : System.Web.UI.Page
    {
        ...
        private void BindData()
        {
            ...
            chkDatabases.DataSource = SctHelper.GetPublishingTargets();
                    ...
                    foreach (string dbName in databaseNames)
                    {
                        ...
                        foreach (Language language in languages)
                        {
                            ...
                            try
                            {
                                ...
                                sourceItem = SctHelper.GetItem(itemPath, language);

                                ///proceed to publish if it is a template or it has versions
                                if (sourceItem != null)
                                {
                                    if (sourceItem.Fields[SctConstants.Templates.Publishing.Fields.NeverPublish].Value != SctConstants.One)
                                    {
                                        hasWorkflow = SctHelper.HasWorkflow(sourceItem);
                                        ...
                                        if (excludeItemsWithWorkflow && !itemPath.StartsWith(SctConstants.Paths.Templates))
                                        {
                                            if (hasWorkflow)
                                            {
                                                isExcluded = true;
                                        ...
                                        if (sourceItem != null)
                                        {
                                            if (hasWorkflow)
                                            {
                                                ...
                                                workflowId = sourceItem.Fields[SctConstants.Templates.Workflow.Fields.Workflow].Value;
                                                SctHelper.RemoveWorkflow(sourceItem, language);
                                            }

                                            publishResult = SctHelper.PublishItem(sourceItem, dbName, false);
                                            ...
                                            if (hasWorkflow)
                                            {
                                                ...
                                                SctHelper.UpdateFieldValues(sourceItem, new List<KeyValuePair<ID, string>> {
                                                    new KeyValuePair<ID, string> ( SctConstants.Templates.Workflow.Fields.Workflow, workflowId ),
                                                    new KeyValuePair<ID, string> ( SctConstants.Templates.Workflow.Fields.WorkflowState, string.Empty )
                                                });
                                            }
                                        }
                                    ...
                                    foreach (ZipArchiveEntry zae in xmls)
                                    {
                                        ...
                                        if (!string.IsNullOrEmpty(xmlText))
                                        {
                                            ...
                                            if (root != null && root.Attributes["id"] != null)
                                            {
                                                ...
                                                {
                                                    Item item = SctHelper.Databases.masterDb.GetItem(itemId);
            ...
            try
            {
                if (parentItemPaths != null && parentItemPaths.Count > 0)
                {
                    ...
                    foreach (string itemPath in parentItemPaths)
                    {
                        ...
                        masterDbItem = SctHelper.Databases.masterDb.GetItem(itemPath);
